...
class PortalSession:
    ...
    async def on_event(self, event: str, message):
        if found := bartholos.EVENTS.get(event, None):
            try:
            ...
            logging.warning(f"Event {event} not found in {bartholos.EVENTS}")
    ...
    async def start(self):
        ...
        await self.sio.connect(
            bartholos.SETTINGS.PORTAL_URL_TO_GAME, wait=True, headers=headers
        )
