...
import origin
from origin.utils.utils import import_from_module, callables_from_module
...
class Core:
    ...
    def copyover(self):
        data_dict = dict()
        for k, v in origin.SERVICES.items():
            v.do_copyover(data_dict)

        for func in origin.HOOKS["copyover"]:
            func(data_dict)
    ...
    async def setup(self):
        ...
        try:
            ...
            for k, v in self.get_setting("CLASSES", dict()).items():
                origin.CLASSES[k] = import_from_module(v)
