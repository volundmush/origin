...
def class_from_module(path, defaultpaths=None, fallback=None):
    ...
    for testpath in paths:
        ...
        try:
            if not importlib.util.find_spec(testpath, package="origin"):
                continue
        except ModuleNotFoundError:
            continue

        try:
            mod = importlib.import_module(testpath, package="origin")
        except ModuleNotFoundError:
...
def crop(text, width=None, suffix="[...]"):
    ...
    """
    width = width if width else origin.GAME.settings.CLIENT_DEFAULT_WIDTH
    ltext = len(text)
    if ltext <= width:
        return text
    else:
